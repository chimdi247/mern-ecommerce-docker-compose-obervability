...
import {
  ...
  ordersPaidCounter,
  revenueCounter,
  paymentSignatureInvalidCounter,
  checkoutFailedCounter,
} from "../../otel/business-metrics";
...
customerCheckoutRouter.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!foundCart.items.length) {
      checkoutFailedCounter.add(1, { reason: "empty_cart" });
    ...
    const items = foundCart.items.map((cartItem) => {
      ...
      if (!product || product.status !== "active") {
        checkoutFailedCounter.add(1, { reason: "product_unavailable" });
        throw new AppError(400, "One or more cart items are not avaibale");
      }

      if (product.stock < cartItem.quantity) {
        checkoutFailedCounter.add(1, { reason: "out_of_stock" });
...
customerCheckoutRouter.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (signature !== razorpaySignature) {
      paymentSignatureInvalidCounter.add(1);
    ...
    ordersPaidCounter.add(1);
    revenueCounter.add(toSubUnits(foundOrder.totalAmount));
